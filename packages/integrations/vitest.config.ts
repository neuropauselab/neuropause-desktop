import { defineConfig } from 'vitest/config';
import { resolve } from 'node:path';

export default defineConfig({
  resolve: {
    alias: {
      '@neuropause/runtime': resolve(__dirname, '../runtime/src/index.ts'),
      '@neuropause/ai-runtime': resolve(__dirname, '../ai-runtime/src/index.ts'),
      '@neuropause/connectors': resolve(__dirname, '../connectors/src/index.ts'),
      '@neuropause/persistence': resolve(__dirname, '../persistence/src/index.ts'),
      '@neuropause/cloud-core': resolve(__dirname, '../cloud-core/src/index.ts'),
      '@neuropause/shared-cloud': resolve(__dirname, '../shared-cloud/src/index.ts'),
    },
  },
  test: {
    environment: 'node',
    include: ['src/**/*.test.ts'],
    server: { deps: { inline: ['@electric-sql/pglite'] } },
    testTimeout: 20000,
  },
});
