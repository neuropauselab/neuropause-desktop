...
/* ── Running Sessions ──────────────────────────────────────────────────────── */
export function RunningSessionsCard({
  sessions,
  onOpenApp,
}: {
...
export function ProductivityCard({ summary }: { summary: ProductivitySummary }): JSX.Element {
  return (
    <Card>
...
export function PendingTasksCard({ tasks }: { tasks: PendingTask[] }): JSX.Element {
            ...
            >
              <span
                className={cn(
                  ...
                  isDone
                    ? 'border-sysgreen bg-sysgreen text-white'
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-faint">{app.name}</span>
              </span>
...
export function RecentActivityCard({ events }: { events: ActivityEvent[] }): JSX.Element {
  return (
    <Card>
          ...
          return (
            <div key={e.id} className="relative flex items-start gap-3 py-1.5">
              ...
              <div className="min-w-0 flex-1 pt-1">
                <div className="truncate text-base">{e.title}</div>
                <div className="truncate text-xs text-faint">
    ...
    <Card>
      ...
      <div className="flex flex-col gap-2.5">
        {items.map((r) => (
          <div
            key={r.id}
            className="rounded-xl border border-[var(--hairline)] [background:var(--fill-1)] p-3"
          >
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 text-xs leading-snug text-muted">{r.detail}</div>
              </div>
            </div>
