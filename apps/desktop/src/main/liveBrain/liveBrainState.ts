 ...
 * It reads no store: `actionRecord.query` is async + disk-backed, so the production call-site
 * does the query and passes the already-scoped `ActionRecord[]` in (see the grounding artifact).
 *
...
export interface LiveBrainState {
  ...
  readonly sections: {
    ...
    readonly authority: StateSection;
...
const VERIFIED_SUCCESS = 'VERIFIED_SUCCESS';
const VERIFIED_FAILURE = 'VERIFIED_FAILURE';
...
export function composeLiveBrainState(inputs: LiveBrainInputs): LiveBrainState {
  ...
  if (wsScope !== null && capScope !== null && wsScope !== capScope) {
    conflicts.push({
  ...
  const scopeResolved = wsScope === true || capScope === true;
  ...
  const routedCaps = new Set((inputs.capabilities?.routes ?? []).map((r) => r.capability));
  for (const rec of inputs.actions) {
    if (routedCaps.has(rec.actionId) && rec.verification?.terminal === VERIFIED_FAILURE) {
      conflicts.push({
        about: `capability "${rec.actionId}" usability`,
        claims: [
          { source: 'L4 capabilities', says: 'routed (a governed path exists)' },
          { source: 'S34a evidence', says: `last governed use verified ${VERIFIED_FAILURE}` },
        ],
      });
      conflicted.add('capabilities');
  ...
  // ── Purpose-bound: L3 discovery ───────────────────────────────────────────────────────
  const discovery: StateSection =
    inputs.discovery === null
      ? sec('UNAVAILABLE', 'no discovery run in scope', ['L3'])
      : sec('KNOWN', `${inputs.discovery.results.length} discovery results`, ['L3'], {
          results: inputs.discovery.results,
        });

  // ── S34a evidence (VERIFIED when an independent success terminal exists) ───────────────
  const verifiedSuccess = inputs.actions.filter((a) => a.verification?.terminal === VERIFIED_SUCCESS).length;
  const evidence: StateSection = conflict('evidence')
    ? sec('CONFLICTING', 'evidence contradicts a routed capability — see conflicts', ['S34a'])
    : inputs.actions.length === 0
      ? sec('UNAVAILABLE', 'no governed actions recorded yet', ['S34a'])
      : verifiedSuccess > 0
        ? sec('VERIFIED', `${inputs.actions.length} recorded, ${verifiedSuccess} independently verified`, ['S34a'], {
            actions: inputs.actions.length,
            verifiedSuccess,
          })
        : sec('KNOWN', `${inputs.actions.length} governed actions recorded`, ['S34a'], {
            actions: inputs.actions.length,
          });

  // ── Derived: authority (a governed path == a routable capability; traces to L4) ───────
  const authority: StateSection =
    inputs.capabilities === null || !inputs.capabilities.scopeResolved
      ? sec('UNAVAILABLE', 'no tenant scope — governed authority undetermined', ['L4'])
      : sec(
          'KNOWN',
          `${inputs.capabilities.routes.length} capabilities have a governed authority path`,
          ['L4 (derived)'],
        );
  ...
  const verifiedFailures = inputs.actions.filter((a) => a.verification?.terminal === VERIFIED_FAILURE).length;
  const incidentCount = conflicts.length + verifiedFailures;
  const incidents: StateSection =
    incidentCount === 0
      ? sec('KNOWN', 'no incidents (no conflicts, no verified failures)', ['derived'])
      : sec('CONFLICTING', `${conflicts.length} conflict(s), ${verifiedFailures} verified failure(s)`, ['derived'], {
          conflicts: conflicts.length,
          verifiedFailures,
        });

  // ── Derived: pending work (unresolved verifications + non-consent-ready purpose) ──────
  const unresolvedVerification = inputs.actions.filter((a) => a.verification === null).length;
  const purposeIncomplete = inputs.purpose !== null && inputs.purpose.state !== 'CONSENT_READY' ? 1 : 0;
  const pendingCount = unresolvedVerification + purposeIncomplete;
  const pendingWork: StateSection =
    pendingCount === 0
      ? sec('KNOWN', 'no pending work', ['derived'])
      : sec('UNKNOWN', `${unresolvedVerification} action(s) awaiting verification` +
          (purposeIncomplete ? ', 1 purpose not yet consent-ready' : ''), ['derived'], {
          unresolvedVerification,
          purposeIncomplete,
        });

  // ── Derived: health (rollup; UNKNOWN never renders as "okay") ─────────────────────────
  // Ambient substrates always count; purpose-bound ones count ONLY when a purpose is in
  // scope — an absent purpose is "not applicable", never a health problem.
  const healthSections: StateSection[] = [workspace, capabilities, evidence, authority];
...
function healthSummary(c: Certainty): string {
  switch (c) {
    case 'CONFLICTING':
      return 'attention — substrates disagree; conflicts surfaced, not reconciled';
    case 'UNAVAILABLE':
      return 'partially blind — one or more substrates cannot be read';
    case 'UNKNOWN':
      return 'incomplete — some state is undetermined (NOT assumed okay)';
    case 'VERIFIED':
      return 'corroborated — observed state is independently verified';
    default:
      return 'observed — all read substrates resolved';
  }
}
