 ...
 */
import type { ObservabilityOverview, ObsSubsystem, ObsSubsystemHealth, SecurityEvent, UsagePoint } from '@neuropause/shared';
...
export function buildObservability(input: ObsInput): ObservabilityOverview {
  ...
  const syncHealth: ObsSubsystemHealth = !input.syncOnline ? 'down' : input.syncPending > 0 ? 'degraded' : 'healthy';
  ...
  const subsystems: ObsSubsystem[] = [
    ...
    { id: 'sync', label: 'Synchronization', status: syncHealth, metric: input.syncDomains, unit: 'domains', detail: input.syncOnline ? `${input.syncPending} changes pending` : 'Offline' },
