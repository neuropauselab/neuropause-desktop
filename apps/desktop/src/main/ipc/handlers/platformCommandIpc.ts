        ...
        const read =
          request.operation === 'QueryDeliveryOperations'
            ...
            : request.operation === 'QueryOperationalExceptions'
              ...
              : request.operation === 'QueryReliabilitySummary'
                // S122 — operational reliability posture over the SAME per-tenant journal records; pure,
                // deterministic, no verdict without a caller-supplied SLO objective (no invented policy).
                ? buildReliabilitySummary(deps.journal, principal.tenantId, params)
                : request.operation === 'QueryEvidenceSearch'
                  // S125 — deterministic lexical/filter search over the SAME per-tenant committed-command
                  // history + verified inbound lineage. Pure, read-only, bounded, credential-free.
                  ? buildEvidenceSearch(deps.journal, platformBusRef.current ?? undefined, principal.tenantId, params)
                  : request.operation === 'QueryEvidenceTrace'
