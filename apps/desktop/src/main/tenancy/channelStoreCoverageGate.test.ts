 ...
 */
const SENSITIVE_BASELINE = 200; // +2 NP-RELEASE-043 §11: AssistantAsk + AssistantCancel left the PUBLIC allowlist (tenant-scoped retrieval + billable model calls behind an unauthenticated channel) and are gated workspace:read. AssistantAsk is DECLARED with the channel — it mutates assistant-conversations (see DECLARED_BASELINE 6→7). AssistantCancel is gated-but-UNDECLARED by design: it clears the caller's in-flight request from memory and reaches no store, so a declaration would be fiction — the FG-1/Slice-10 precedent. // +1 FG-S80b: kpi:capture gated (intelligence:read, mirrors ExecutiveCenterSnapshot). DECLARED with the channel this time — it reaches real stores (see DECLARED_BASELINE). // +1 FG-1 (Wave-2 Slice 10): capability:m365.propose gated (connectors:manage). Gated-but-UNDECLARED by design: an unhandled channel reaches no store, so a declaration would be fiction — declareChannelResource + DECLARED_BASELINE 3→4 land in Slice 11 with the handler. // +1 S115 FG-S114-AUDIT-STATUS: security:auditIntegrity.status gated (operations:read). DECLARED with the channel — it reaches the workforce-governance-audit store (read-only integrity status).
 ...
 */
const DECLARED_BASELINE = 7; // +1 NP-RELEASE-043 §11: assistant:ask declares its mutate of assistant-conversations (ask() loads/creates the conversation and persists both turns). // +1 FG-S80b: kpi:capture declares its mutate of kpi-snapshots/kpi-exceptions (a governed write channel — coverage improves, not just the surface). // +1 FG-2 (Wave-2 Slice 11): capability:m365.propose declares its connector-accounts read (handler landed) // +1 S115: security:auditIntegrity.status declares its workforce-governance-audit read (handler landed)
