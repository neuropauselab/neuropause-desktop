...
export const UNENFORCED: readonly { store: string; status: TenantMigrationStatus; note: string }[] = [
  ...
  },
  {
    ...
    status: 'PARTIAL',
    note: 'P13C N3 — the subsystem had NO tenant dimension: `SandboxWorkspace` carried no owner, so nothing downstream could be scoped even in principle. Every persisted type now carries `tenantId`; the seam lives on the shared `PersistentStore` base so all subclasses inherit one implementation and a SIXTH store starts out denying. Writes stamp from the resolved tenant and REFUSE when none resolves; reads filter; a foreign id reads as absent on workspace, scenario, execution, artifact and dataset alike, including the sharpest one — `artifacts.get`, whose `inline` carries the full result and report JSON. An omitted `workspaceId` now narrows within the tenant instead of widening to the install, which was the bypass. `ensureDefault()` returned THE FIRST WORKSPACE ON THE INSTALL and is now first-of-mine. The engine keeps a shared queue on purpose — otherwise a tenant\u2019s runs stall until that tenant acts — but each execution runs under a principal built from its own row, and an unowned row is failed rather than run; dataset attach is resolved through the scoped accessor, so B cannot attach A\u2019s fixture. The event broadcast carries its owner. PARTIAL, not COMPLETE: rows written before P13C are unresolved and visible to nobody, and the execution and validation retention caps remain INSTALL-WIDE, so a noisy tenant can still evict another\u2019s history \u2014 a fairness problem, not a disclosure.',
  },
  {
    store: 'sandbox validation runs + perf benchmarks',
    status: 'PARTIAL',
    note: 'P13C \u2014 ADDED AFTER A SECOND SWEEP, having been missed by the first. `ValidationRunStore` and `BenchmarkStore` extend the same `PersistentStore` as the five stores above and gained the same seam in the same change; nobody bound them, so they stayed open while their siblings closed. `hasScope()` existed with zero callers, which is exactly how that happens \u2014 `initSandbox` now THROWS on an unbound store. Reachable before the fix: `sandbox:validation.run.get` took a runId from the payload and returned a cached `ValidationRunDetail` whose certification report carries that tenant\u2019s live executive KPI figures plus ready-made exports; `.summary` and `.dashboard` returned every tenant\u2019s run history, which is where those runIds came from; and a benchmark BASELINE is copied verbatim into the next run\u2019s regression findings, so one tenant\u2019s measured latency printed inside another\u2019s report. `sandbox:read` is in the base read-only role, so every member of every tenant could call all three. Both types now carry `tenantId`, both stores are bound, the outputs cache is keyed by (tenant, runId) and the in-flight pipeline is tracked per tenant.',
  },

  {
    store: 'assistant conversations (conversationStore)',
    status: 'PARTIAL',
    note: 'P13C N7 \u2014 conversation bodies carry assistant answers SYNTHESISED FROM TENANT DATA, and the store had no boundary. `list(null)` meant no filter \u2014 every conversation on the install \u2014 and the request schema makes `workspaceId` nullable AND optional, so `{}` was a valid payload that returned everything; `get(id)` selected by bare id, so knowing a uuid was the authorization; and `upsert` was a write-side IDOR that could overwrite another tenant\u2019s title and message history. All three are now scoped, and `null` means "mine" rather than "all" \u2014 there is deliberately no argument that widens it. The channels were also on the PUBLIC allowlist (no auth, no permission) and have been moved into RBAC under `dashboard:read`, the universal signed-in read scope this codebase already uses for per-user surfaces whose owner is resolved server-side; `assistant:ask` and `assistant:cancel` remain public deliberately. Pre-P13C conversations are unresolved and visible to nobody. PARTIAL: `assistant:cancel` still resolves an in-flight turn by conversation id with no tenant check \u2014 availability only, nothing is returned.',
  },

  {
    store: 'federation (runtime / exchange / governance)',
    status: 'PARTIAL',
    note: 'P13C ROUND 4 — S-10. ADDED TO THIS INVENTORY, having been absent from it entirely, which is most of why five sweeps never opened it. The subsystem had NO TENANT DIMENSION: `FederationRuntimeStore` and `GlobalGovStore` took their home organization as a CONSTRUCTOR ARGUMENT wired to the seeded ORG_ID, so home was a property of the machine. Every tenant saw the same org directory, invitations, trust, shares, artifacts, policies, approvals and federated audit; `revokeShare`, `respondInvitation`, `rollback`, `setVerification`, `setScope`, `publishVersion`, `setPolicyEnabled` and `resolveApproval` each took a bare payload id; and the publish handler passed the literal ORG_ID as `publisherOrg`, so every tenant’s artifact was Ed25519-SIGNED as the seeded organization. Federation is now RELATIONSHIP-scoped rather than owner-scoped (`FederationBoundary`): a record names two organizations and the caller must be one of them, so a share is visible to owner AND participant and to nobody else — a plain tenantId filter would have hidden it from the party it exists for. The org directory is DERIVED from relationships instead of listed. Artifact visibility follows the publisher’s scope (private / partner-by-trust / regional-by-region / public); installation records the installer without rewriting the publisher. PARTIAL, and these are the reasons: `setVerification` is publisher-only, which closes the cross-tenant hole and leaves SELF-ATTESTATION (a publisher can still call its own artifact "official") — a platform verifier role is a product decision, not a patch; `ownerOrg` is optional so pre-Round-4 rows have no party, and unowned POLICIES therefore stop being evaluated, which is fail-open on a control rather than on data; and `inviteOrg` derives the target organization id from a display name, so the only ids it can address are hyphen-slugs — real federation between two UI-created organizations is not currently expressible.',
  },
  {
    store: 'exchange packs (packsStore)',
    status: 'PARTIAL',
    note: 'P13C ROUND 4 — F9. ADDED TO THIS INVENTORY. `publish()` stamped `publisherOrgId` from the seeded ORG_ID — every row LOOKED owned and the value was a constant nothing read back, which is the shape an auditor asking "do these rows have an owner?" gets a yes from. `list()` and `stats()` returned every organization’s packs on `developer:read`, `importPack(id)` mutated another tenant’s counters, and `remove(id)` was `packs.delete(id)` on a bare payload id — an unrecoverable cross-tenant delete of published content. The store was also absent from the binding block that binds its three siblings while being loaded beside them. Now owner-scoped and bound. PARTIAL: `ExchangePack` has NO visibility scope field (unlike a federation artifact), so owner-only is the only safe default and there is currently no way to publish a pack to a community catalogue. Adding a scope field is the fix and it is a product change.',
  },
  {
