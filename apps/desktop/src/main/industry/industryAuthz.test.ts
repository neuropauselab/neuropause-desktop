...
describe('withIndustryAuthz', () => {
  it('gates all 7 industry channels with industry:read and requireAuth', () => {
    const defs = INDUSTRY_CHANNELS.map((channel) => ({
      channel,
      schema: {} as never,
      handler: () => null,
    }));
  ...
  it('classifies exactly the industry invokable channels (registry bijection, all read-only)', () => {
    ...
    expect(industryInvokable.length).toBe(7);
    // Every invokable industry channel is classified as a read…
    for (const c of industryInvokable)
      expect(INDUSTRY_CHANNEL_PERMISSIONS[c]).toBe('industry:read');
  ...
  it('throws at startup on an unclassified channel (never silently unguarded)', () => {
    expect(() =>
      withIndustryAuthz([
        { channel: IpcChannel.WorkforceWorkers, schema: {} as never, handler: () => null },
      ]),
  ...
  it('preserves other handler fields (schema, audit)', () => {
    ...
    const [g] = withIndustryAuthz([
      { channel: IpcChannel.IndustryOverview, schema, handler: () => 1, audit: true },
    ]);
