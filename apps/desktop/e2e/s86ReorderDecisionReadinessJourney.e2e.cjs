#!/usr/bin/env node
/**
 * ERP S86 — GOVERNED REORDER DECISION-READINESS journey in the REAL Electron runtime.
 *
 * Proves, entirely through the governed `enterprise:module.*` bridge (window.neuropause.invoke):
 *   product (with a purchase cost) → receive stock → ship (demand history) → generate the
 *   reorder-DECISION register (S85 recommendation + estimated order value + required approval steps
 *   + blockers + fail-closed execution readiness) → governed read → deterministic regeneration →
 *   product/shipping stores read-only.
 *
 * Then the CRITICAL NEGATIVES — a decision report is DECISION INTELLIGENCE ONLY, NON-EXECUTING:
 ...
 *   → execution stays BLOCKED on every generated row (no automatic execution path)
 *
 * Build first:  env -u NP_E2E_BUILD npx electron-vite build --outDir "$PWD/out-seam-s86"
 * Run:          NODE_PATH="$(git rev-parse --show-toplevel)/node_modules" node e2e/s86ReorderDecisionReadinessJourney.e2e.cjs
 */
const { _electron: electron } = require('playwright-core');
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const ALT_MAIN = path.join(path.resolve(__dirname, '..'), 'out-seam-s86/main/index.js');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
function out(k, v) { console.log(`S86 ${k} = ${JSON.stringify(v)}`); }
function fail(m) { console.error(`S86 FAIL: ${m}`); process.exitCode = 1; throw new Error(m); }
...
async function main() {
  if (!fs.existsSync(ALT_MAIN)) fail(`alternate build missing: ${ALT_MAIN} (build out-seam-s86 first)`);
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'np-s86-'));
  const logs = [];
  ...
  try {
    ...
    // 2. generate the reorder-DECISION register
    const v1 = await create('inventory-reorder-decision', { asOfDate: new Date().toISOString().slice(0, 10) });
    assert(v1.ok, 'reorder-decision register generated');
    const row = JSON.parse(String(v1.record.fields.rows)).find((r) => r.sku === 'SKU-1');
    assert(row && row.readinessStatus === 'READY_FOR_OPERATOR_REVIEW', 'SKU-1 readiness = READY_FOR_OPERATOR_REVIEW (triggered + purchaseCost > 0)');
    // suggestedQuantity 430 (target 500 − position 70) from the canonical engine; × purchaseCost 4 = 1720
    assert(row.suggestedQuantity === 430 && row.estimatedOrderValue === 1720, 'estimatedOrderValue 1720 (430 × purchaseCost 4), from the canonical qty × canonical cost');
    // 1720 < 10000 → only the always-on manager step (from the existing DEFAULT_SPEND_POLICY, not invented)
    assert(Array.isArray(row.requiredApprovalSteps) && row.requiredApprovalSteps.length === 1 && row.requiredApprovalSteps[0] === 'Manager approval', 'requiredApprovalSteps = [Manager approval] (from the existing spend policy)');
    assert(row.executionReadiness === 'BLOCKED_UNDEFINED_POLICY', 'executionReadiness fail-closed (BLOCKED_UNDEFINED_POLICY)');
    assert(num(v1, 'reorderRequiredCount') === 1 && num(v1, 'readyForReviewCount') === 1, 'reorderRequiredCount 1, readyForReviewCount 1');
    assert(String(v1.record.fields.executionBlocked).includes('policy-undefined'), 'snapshot marks execution blocked (policy-undefined)');

    // 3. deterministic regeneration
    const v2 = await create('inventory-reorder-decision', { asOfDate: new Date().toISOString().slice(0, 10) });
    assert(String(v2.record.fields.rows) === String(v1.record.fields.rows), 'deterministic regeneration: rows byte-identical');

    // 4. CRITICAL NEGATIVES — generation is decision intelligence only, NON-EXECUTING
    ...
    const allBlocked = JSON.parse(String(v1.record.fields.rows)).every((r) => r.executionReadiness === 'BLOCKED_UNDEFINED_POLICY');
    assert(allBlocked, 'execution BLOCKED on every generated row (no automatic execution path)');

    // 5. governed read returns the immutable register history (tenant-scoped)
    assert((await listOf('inventory-reorder-decision')).length === 2, 'governed read returns both decision reports');

    out('RESULT', 'S86 reorder-DECISION readiness intelligence VERIFIED in the real Electron runtime (product → receive → ship → reorder decision → deterministic regeneration → governed read), decision intelligence only — NO PR, NO PO, NO inventory mutation, NO GL, execution blocked on every row');
  } finally {
