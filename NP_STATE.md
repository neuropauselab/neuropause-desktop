...
- **BRAIN-1 LANDED (②③④, non-frozen, no FG gate).** ① hostile-adapter CI gate (draft model gains no authority). ② `ai/brain/mailDraftGateway.ts` (model → zod → 1 repair → referenceDrafter fallback). ③ assistantService via `servingDraftMailer()` (reference-only). ④ `draftEval.ts` harness + eval report (honest zero-model baseline, 0 candidates) + DECISIONS D-13 + badge (names a model only when one serves) + LiteLLM reserved. Fixed a latent S17 `cssTokens` failure (LocalModeBanner `--surface`). Full main **8784/3** · UI **259** · typecheck+lint clean. Draft lane serves `referenceDrafter` until eval clears + operator go. **Next:** §5 roadmap-amendment draft (approval req.); S34a; S19. **Deferred (operator keyboard):** S15 containment — revoke consent + delete app registration + `rm -rf ~/Library/Application Support/NeuroPause-S15` (evidence copied out).
