...
is x64-ONLY (nsis/zip/portable x64) - electron-builder.yml,
unchanged. The drafted pilot config inherited that baseline until
the human decision below was supplied; the pilot config was then
revised to arm64 (v2).

Record history: draft v1 of this record stated "HUMAN ARM64
DECISION = NOT_SUPPLIED". That state is SUPERSEDED by the following
signed decision, captured verbatim:

----------------------------------------------------------------
PILOT_ARCHITECTURE = arm64

DESIGNATED_PILOT_ENVIRONMENT = Non-production Windows 11 ARM64
pilot environment designated by Saurabh Patel, isolated from
production infrastructure and public release infrastructure.
