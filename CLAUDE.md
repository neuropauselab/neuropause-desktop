...
- **Slice 16 — read-back oracle: MOCK-PROVEN, real run pending.** `verifyEffect` (pure state machine: EXECUTED_ACK → VERIFY_PENDING → VERIFIED_SUCCESS | VERIFY_FAILED → UNKNOWN → HOLD; corroborated match — never id alone; idempotent; UNRESOLVED never auto-promotes) + `m365ReadBack` (in-session READ-ONLY Graph reader) + `s16VerifyRun` (compile-gated in-session runner). 17 oracle pins incl. fault injection (dropped response → HOLD → resolves later). No FG-5 (verification state is a separate non-frozen store, D-10). verify-e2e-strip PASS. **The first REAL VERIFIED_SUCCESS is the operator's in-session run** (`NEUROPAUSE_VERIFY_S15=1`, S15 profile) — reads back the S15 message, logs TERMINAL + internetMessageId. **Next:** that run → fold the terminal truth + match tuple into evidence; then containment; then S17 local-first, S34 action-trace.
