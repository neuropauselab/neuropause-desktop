...
- Landed: Slices 1–13. Slice 11 = FG-2: `runtimeCore` registration (channel LIVE-REGISTERED, data-only). Slice 12 = first production feed → `M365WritePanel` (dev-triggered) + typed refusal UI + comma hardening; NON-frozen via `rawInvoke` (D-6). Slice 13 = AI structured mail.send intent generator (`assistantMailIntent.ts`, safety model-INDEPENDENT: recipient literalism, context-inert, trigger discipline, deny-by-default) + **FG-3** (additive optional `AssistantEnvelope.mailIntent` field) + the assistant→panel wiring (ask() → generator → `mailIntent` → renderer hand-off → S12 feed → the ONE `M365WritePanel`; DECISIONS D-7). FG-3 INTACT bracket 92a99c8 → de64dd0 → 1ed71cc. Golden set 37 cases per-category (positive 10/10, ambiguous 8/8, hostile 9/9, out-of-scope 10/10; binary safety = zero pass-throughs).
- Suites (RUN against BASELINE-52d9a12099f3): full main **8708 passed / 3 skipped** (821 files) · UI **254** (34 files) · typecheck + lint clean (incl. frozen `assistant.ts`).
- Next: **Slice 14** — full mock E2E in the REAL Electron app (Playwright): typed NL → intent → propose → panel → human-style confirm → certified executor → mock Graph → admission, with a captured recording. ⛔ hard stop before anything real (S15). S13 is TEST-VERIFIED at component/jsdom level; the real-app run is S14 (not collapsed).
