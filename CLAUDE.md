...
- Landed: Slices 1–12 + Slice-13 SAFETY CORE. Slice 11 = FG-2: `runtimeCore` registration (channel LIVE-REGISTERED, data-only), INTACT 2668ab8 → 5534c45 → aff5d13. Slice 12 = first production feed → `M365WritePanel` (dev-triggered) + typed refusal UI + comma hardening; NON-frozen via `rawInvoke` (D-6). Slice 13 core (`c15bec2`) = `assistantMailIntent.ts` deterministic mail.send intent generator: recipient literalism, context-inert, trigger discipline, deny-by-default scope — safety is model-INDEPENDENT (guards wrap an untrusted drafter). 38 tests: golden set 37 cases per-category (positive 10/10, ambiguous 8/8, hostile 9/9, out-of-scope 10/10; binary safety = zero pass-throughs) + adversarial-model + permanent hostile-synced gate.
- Suites: main core RUN-against-BASELINE-329a95225ea7 (assistantMailIntent 38/38) · full main 8661/3 (818) + this slice's tests · UI 250 · typecheck + lint clean.
- **In flight: Slice 13 stopped at FG-3** — the assistant→panel carrier needs an additive optional `AssistantEnvelope.mailIntent` field (frozen `packages/shared`). Gate presented (verbatim one-field diff + threat analysis + verification plan; DECISIONS D-7, rule-4 ONE-SURFACE). Awaiting token `AUTHORIZED: FG-3 — AssistantEnvelope.mailIntent additive optional field, per gate doc`. Non-frozen wiring (AssistantService integration + renderer handoff → S12 feed → M365WritePanel) prepped to land on token.
...
- **FG-3 (OPEN — presented, awaiting token):** additive optional `AssistantEnvelope.mailIntent?: { to: string[]; subject: string; body: string } | null` field in `packages/shared/src/types/assistant.ts` — the Slice-13 assistant→panel carrier (one surface via the S12 feed; DECISIONS D-7). Gate doc: `…SLICE-13-FG3-ASSISTANT-MAILINTENT-CARRIER-GATE.md`. Token: `AUTHORIZED: FG-3 — AssistantEnvelope.mailIntent additive optional field, per gate doc`.
- **FG-4 (anticipated):** frozen admission-record field for verification state, if S16 needs it.
